...
function ttSubjectsFor(collegeId, branchId, semester) {
  const sem = ttLookupSemester(collegeId, branchId, semester);
  if (!sem) return [];
  const set = new Set();
  Object.values(sem.sections).forEach(days => {
    Object.values(days).forEach(dayArr => {
      (dayArr || []).forEach(slot => {
        if (slot.type === "break" || !slot.subject) return;
        const normalized = slot.subject.replace(/\s+G\d+$/i, "").trim();
        set.add(normalized);
      });
    });
  });
